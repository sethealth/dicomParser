...
function readSequenceItemImplicit (byteStream, vrCallback) {
  ...
  if (item.length === 4294967295) {
    ...
    parseDicomDataSetImplicit(item.dataSet, byteStream, byteStream.position + item.length, { vrCallback });
  }
...
function readSequenceItemExplicit (byteStream, warnings) {
  ...
  if (item.length === 4294967295) {
    ...
    item.dataSet = new DataSet(byteStream.byteArrayParser, byteStream.byteArray, {});
    parseDicomDataSetExplicit(item.dataSet, byteStream, byteStream.position + item.length);
  }
...
function parseDicom (byteArray, options) {
  ...
  function readDataSet (metaHeaderDataSet) {
    ...
    try {
      if (explicit) {
        parseDicomDataSetExplicit(dataSet, dataSetByteStream, dataSetByteStream.byteArray.length, options);
      } else {
        parseDicomDataSetImplicit(dataSet, dataSetByteStream, dataSetByteStream.byteArray.length, options);
      }
    } catch (e) {
      const ex = {
        exception: e,
  ...
  function parseTheByteStream () {
    const metaHeaderDataSet = readPart10Header(byteArray);
...
export { ByteStream, DataSet, alloc, bigEndianByteArrayParser, createJPEGBasicOffsetTable, explicitDataSetToJS, explicitElementToString, findAndSetUNElementLength, findEndOfEncapsulatedElement, findItemDelimitationItemAndSetElementLength, isPrivateTag, isStringVr, littleEndianByteArrayParser, parseDA, parseDicom, parseDicomDataSetExplicit, parseDicomDataSetImplicit, parsePN, parseTM, readDicomElementExplicit, readDicomElementImplicit, readEncapsulatedImageFrame, readEncapsulatedPixelData, readEncapsulatedPixelDataFromFragments, readFixedString, readPart10Header, readSequenceItem, readSequenceItemsExplicit, readSequenceItemsImplicit, readTag, sharedCopy, version };
